...
/**
 * @author liuyl@risen.com 2018年6月6日上午1:49:21 通过内存映射方式，分块读取大文件
 *
 */
public class MappedBiggerFileReader {
    ...
    public static void main(String[] args) throws IOException {
        MappedBiggerFileReader reader = new MappedBiggerFileReader(new File("D:/1.doc"), 65536);
        long start = System.nanoTime();
        int readLine = 0;
        FileOutputStream fis = new FileOutputStream("D:/2.doc", true);
        while ((readLine = reader.read()) != -1) {
            fis.write(reader.getArray());
            System.out.println(reader.getArray());
        }
        fis.close();
